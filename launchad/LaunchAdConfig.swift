//
//  LaunchAdConfig.swift
//  Tracker
//
//  Created by StephenLouis on 2019/1/14.
...
// MARK: - 完成动画类型
@objc public enum LaunchAnimationType: Int {
    /// 溶解消失
    case crossDissolve
    /// 翻页
    case curlUp
...
// MARK: - 跳过按钮类型
@objc public enum LaunchSkipButtonType: Int {
    /// 无跳过按钮
    case none = 0
    /// 文字跳过
    case text
...
// MARK: - 缓存方式
@objc public enum LaunchAdImageOptions: Int {
    /// 只加载, 不缓存
    case onlyLoad = 0
    /// 先读缓存，再加载图片，刷新缓存
    case refreshCache
    /// 有缓存，读取缓存，不重新加载；没有缓存先加载，并缓存
    case readCache
}

// MARK: - 配置跳过按钮
@objc public class LaunchSkipButtonConfig: NSObject {
    /// frame
    ...
    /// 跳过按钮类型
    @objc public var skipBtnType: LaunchSkipButtonType = .textLeftTimerRight
    /// 圆形进度颜色
